...
const sendOTP = async (req , res) => {
    try{
        ...
        res.status(200).json({msg: 'OTP has been send to your email.'});
    }catch(err){
...
const resendOTP = async (req , res) => {
    try{
        ...
        await transpoter.sendMail(mailOptions);
        res.status(200).json({msg: 'OTP has been resend to your email.'});
    }catch(err){
        res.status(500).json(err);
    }
}
    ...
    try{
        ...
        if(doc){
            if(Date.now() < doc.expireAt){
                ...
                res.status(400).json({msg: 'OTP has expire.'});
            }
        }else{
